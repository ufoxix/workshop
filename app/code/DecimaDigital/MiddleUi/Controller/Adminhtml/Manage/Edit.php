    ...
    public function execute()
    {
        ...
        if ($entityId) {
            try {
                ...
                $resultPage->addBreadcrumb(__('Configure MiddleUi'), __('Configure MiddleUi'));
                $resultPage->getConfig()->getTitle()
                    ->prepend(__('Edit Configure MiddleUi Configuration'));
            } catch (\Exception $e) {
                $this->messageManager->addErrorMessage($e->getMessage());
                return $resultRedirect->setPath('*/*/index');
            }
            $resultPage->getConfig()->getTitle()
                ->prepend(__('Edit MiddleUi Configuration for "%1"', $simpleUiEntity->getName()));
            return $resultPage;
        } else {
            $this->_view->getPage()->getConfig()->getTitle()->prepend(__('New MiddleUi Configuration'));
        }
        return $resultPage;
    }
}
